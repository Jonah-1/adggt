...
DGDA 在 Aggregator 的 `frame_blocks` 和 `global_blocks` 中，将每个 `nn.Linear` 层替换为 `DynamicGatedLoRA` 包装器。原始权重完全冻结，只训练轻量 LoRA 适配器参数。
...
每个 Block 有 4 个 Linear 被替换，24 层 × 2（frame+global）= **192 个 DGDA 模块**级联。为防止累计误差：
...
难点：`DynamicGatedLoRA.forward(x, dynamic_conf)` 需要额外参数，但 `Attention`/`Mlp` 内部调用 `self.qkv(x)` 时只传 `x`——无法修改每个中间层的签名。
...
_prepare_dynamic_conf(dynamic_conf, B, S, P)
        │
        ├─► frame_blocks 所有 DGDA: _dynamic_conf = conf.view(B*S, P, 1)
        └─► global_blocks 所有 DGDA: _dynamic_conf = conf.view(B, S*P, 1)
        │
        ▼
Block.forward(tokens) → Attention 内部: self.qkv(x)
        │
        ▼
...
special tokens（camera / register）的 dynamic_conf 填充为 **0.5**，对两个分支等权。

## 3. 注入位置

每个 Block 内有 4 个 `nn.Linear` 被替换：
...
共 24 层 frame_blocks + 24 层 global_blocks = **48 个 Block × 4 = 192 个 DGDA 模块**。
...
# Step 2: 注入 DGDA（替换 Linear → DynamicGatedLoRA，冻结骨干）
model.enable_dgda(r=8, lora_alpha=16, dropout=0.1, freeze_backbone=True)
...
```python
model = VGGT().to(device)
model.enable_dgda(r=8, lora_alpha=16)      # 先创建 LoRA 结构
checkpoint = torch.load("model_latest.pt", map_location="cpu")
model.load_state_dict(checkpoint, strict=False)  # 再加载权重
```
...
model.load_dgda("dgda_adapter.pth")        # 只加载 DGDA 参数
```
...
以 `embed_dim=1024, r=8` 为例，单个 DGDA 模块的参数：

| 组件 | 参数量 |
|------|--------|
| static_A + static_B | `in_dim×r + r×out_dim` |
| dynamic_A + dynamic_B | `in_dim×r + r×out_dim` |
| static_res_gate + dynamic_res_gate | 2 |
| static_norm + dynamic_norm | `4 × out_dim` |
| **总计 (以 qkv 层为例)** | **~57K** |
